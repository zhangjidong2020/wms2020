...
public class GetAutoCode {
    ...
    private static final String STR_FORMAT = "0000000000"; //需要格式化的流水号规则

    /**
     * @return 获取九位流水号
     */
    public static String getThreePipelineNumbers() {
    ...
    /**
     * @return 获取年月日+九位流水号（根据当天生成的单据数量，生成流水号如：20190522001,20190601001）
     */
    public static String getFourPipelineNumbers() {
        int count = new Random().nextInt(999999999); //总数量：一般在实际业务中是需要在数据库中获取到当前的数据总数量
        // 所要获取的流水编码code
     ...
     * @return 获取年月日+九位流水号（根据当天生成的单据数量，生成流水号如：20190522001,20190601001）
     */
    public static String getPipelineNumbers() {
        int count = new Random().nextInt(1000000); //总数量：一般在实际业务中是需要在数据库中获取到当前的数据总数量
        // 所要获取的流水编码code
        StringBuffer code = new StringBuffer();
        // 设定所需的时间格式
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
    ...
    public static void main(String[] args) {
        System.out.println(GetAutoCode.getFourPipelineNumbers());
    }
}
